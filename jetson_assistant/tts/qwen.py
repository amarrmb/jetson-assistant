        ...
        **kwargs,
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install with: pip install jetson-assistant[qwen]"
            ) from e

        # Check for flash-attn, fall back to SDPA if not available
        try:
            import flash_attn  # noqa: F401
            _attn_impl = "flash_attention_2"
            logger.info("Using flash-attn for Qwen TTS")
        except ImportError:
            _attn_impl = "sdpa"
            logger.info("flash-attn not available, using SDPA for Qwen TTS")
        ...
        # Use flash-attn if available, otherwise fall back to SDPA.
        # The transformers library auto-selects based on availability, but
        # we log which implementation is active for debugging.
        ...
        self._loaded = True
        logger.info("Qwen3-TTS model loaded (attn=%s)", _attn_impl)
