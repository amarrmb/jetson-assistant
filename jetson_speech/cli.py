"""
Command-line interface for Jetson Speech.
"""
...
def serve(
    ...
    if preload_tts:
        os.environ["JETSON_SPEECH_PRELOAD_TTS"] = preload_tts
    if preload_stt:
        os.environ["JETSON_SPEECH_PRELOAD_STT"] = preload_stt
    if warmup:
        os.environ["JETSON_SPEECH_WARMUP"] = "1"

    from jetson_speech.server.app import run_server
...
def tts(
    ...
    from jetson_speech.core.audio import play_audio, save_audio
...
def assistant(
    ...
    Example (fast, server mode):
        ...
        jetson-speech assistant --server --llm-model phi3:mini

    Example (slow, loads models each time):
        jetson-speech assistant --llm ollama --llm-model llama3.2:3b
    """
    from jetson_speech.assistant.core import AssistantConfig, VoiceAssistant
    ...
    if server:
        ...
        console.print("[dim]Make sure server is running: jetson-speech serve[/dim]\n")
    else:
    ...
    try:
        ...
        console.print("  jetson-speech serve --port 8080")
        raise typer.Exit(1)
...
def info():
    ...
    console.print(f"\n[bold]Jetson Speech v{__version__}[/bold]\n")
