"""
Configuration and settings for Jetson Speech.
"""
...
def get_default_cache_dir() -> Path:
    ...
    return Path(os.environ.get("JETSON_SPEECH_MODEL_CACHE", Path.home() / ".cache" / "jetson-speech"))
...
class ServerConfig(BaseModel):
    ...
    host: str = Field(
        default_factory=lambda: os.environ.get("JETSON_SPEECH_HOST", "0.0.0.0")
    )
    port: int = Field(
        default_factory=lambda: int(os.environ.get("JETSON_SPEECH_PORT", "8080"))
    )
...
class TTSConfig(BaseModel):
    ...
    default_backend: str = Field(
        default_factory=lambda: os.environ.get("JETSON_SPEECH_TTS_BACKEND", "qwen")
    )
...
class STTConfig(BaseModel):
    ...
    default_backend: str = Field(
        default_factory=lambda: os.environ.get("JETSON_SPEECH_STT_BACKEND", "whisper")
    )
